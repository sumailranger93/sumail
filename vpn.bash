...
if [[ ! -e /etc/openvpn/server_udp.conf ]]; then
 echo -e "Missing OpenVPN Server config, exiting..."
 exit 1
fi

PRIVATE_IP="$(cat /etc/openvpn/server_udp.conf | grep -i server | sed -e '/cert.*/d' -e '/key.*/d' | awk '{printf $2}')"

CIDR_CLASS=""
if grep -qs 255.255.0.0 /etc/openvpn/server_udp.conf; then
 CIDR_CLASS="16"
elif grep -qs 255.255.255.0 /etc/openvpn/server_udp.conf; then
 CIDR_CLASS="24"
fi
